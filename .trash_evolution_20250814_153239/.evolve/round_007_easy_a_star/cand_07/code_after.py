...
def accepts(grammar: Dict, s: str) -> bool:
    ...
    if s == "a" and start in rules:
        for production in rules[start]:
            if production == "a" or (production.startswith("a") and production[1:] in rules):
                return True
    # Simple case: if string is just "a" and grammar allows it
