...
def accepts(grammar: Dict, s: str) -> bool:
    ...
    if s == "" and "" in rules.get(start, []):
        return True
    # For non-empty strings, try matching the first character
    if s and s[0] == 'a' and 'aS' in rules.get(start, []):
        # Recursively check the rest of the string with the same grammar
        return accepts(grammar, s[1:])
